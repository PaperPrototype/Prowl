...
internal sealed class GlTexture : InternalTexture
{
    public uint Handle;
    ...
    public static unsafe InternalTexture CreateTexture(TextureDescription description, void* data)
    {
        ...
        else
        {
            ...
            GraphicUtils.CalculatePitch(description.Format, description.Width, out int bpp);

            // Calculate the number of mip levels if the description's value is 0.
            int mipLevels = description.MipLevels == 0
                ? GraphicUtils.CalculateMipLevels(description.Width, description.Height, description.Depth)
            ...
            if (data != null)
            {
                ...
                for (int a = 0; a < description.ArraySize * (description.TextureType == TextureType.Cubemap ? 6 : 1); a++)
                {
                    ...
                    uint height = (uint) GraphicUtils.Max(description.Height, 1);
                    uint depth = (uint) GraphicUtils.Max(description.Depth, 1);

                    // The loop must run at least once, even if the mip levels are 0.
                    for (int i = 0; i < GraphicUtils.Max(1, description.MipLevels); i++)
                    {
    ...
    public unsafe void Update(int x, int y, int z, int width, int height, int depth, int mipLevel, int arrayIndex, void* data)
    {
        ...
        GraphicUtils.CalculatePitch(description.Format, description.Width, out int bpp);
        uint size = (uint) (GraphicUtils.Max(description.Width, 1) * GraphicUtils.Max(description.Height, 1) *
                            GraphicUtils.Max(description.Depth, 1) * (bpp / 8f));
