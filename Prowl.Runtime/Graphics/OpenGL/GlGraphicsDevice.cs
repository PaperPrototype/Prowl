...
{
    private Silk.NET.Core.Contexts.IGLContext _context;
    ...
    public GlGraphicsDevice(bool es, Silk.NET.Core.Contexts.IGLContext context, Size winSize, GraphicsDeviceOptions options)
    {
        _context = context;
        Gl = GL.GetApi(_context);
        Debug = options.Debug;
        IsES = es;
    ...
    public override void ClearColorBuffer(Color color) =>
        ClearColorBuffer(color.r, color.g, color.b, color.a);

    public override void ClearColorBuffer(Vector4 color) => ClearColorBuffer((float)color.x, (float)color.y, (float)color.z, (float)color.w);
    ...
    private void InvalidateCaches()
    {
        _currentRState = null;
        _currentBState = null;
        GraphicsMetrics.DrawCalls = 0;
        GraphicsMetrics.TriCount = 0;
    }
    ...
    public override InternalTexture CreateTexture<T>(TextureDescription description, T[][] data)
    {
        fixed (void* ptr = GraphicUtils.Combine(data))
            return GlTexture.CreateTexture(description, ptr);
    }
    ...
    public override InternalTexture CreateTexture(TextureDescription description, void* data)
    {
        return GlTexture.CreateTexture(description, data);
    }

    public override InternalShader CreateShader(ShaderAttachment[] attachments)
    {
    ...
    public override void UpdateTexture<T>(InternalTexture texture, int mipLevel, int arrayIndex, int x, int y, int z,
    ...
    public override void UpdateTexture(InternalTexture texture, int mipLevel, int arrayIndex, int x, int y, int z, int width,
        int height, int depth, IntPtr data)
    {
        ((GlTexture) texture).Update(x, y, z, width, height, depth, mipLevel, arrayIndex, data.ToPointer());
    }

    public override void UpdateTexture(InternalTexture texture, int mipLevel, int arrayIndex, int x, int y, int z,
        int width, int height, int depth, void* data)
    {
        ((GlTexture) texture).Update(x, y, z, width, height, depth, mipLevel, arrayIndex, data);
    }

    public override unsafe void GetTextureData(InternalTexture texture, int mipLevel, int arrayIndex, int x, int y, int z, int width, int height, int depth, void* data)
    {
        ((GlTexture) texture).GetData(x, y, z, width, height, depth, mipLevel, arrayIndex, data);
    }
    ...
    public override void SetTexture(uint bindingSlot, InternalTexture texture, SamplerState state)
    {
    ...
    public override void Draw(uint vertexCount)
    {
        ...
        GraphicsMetrics.DrawCalls++;
        GraphicsMetrics.TriCount += vertexCount / 3;
    }
    ...
    public override void DrawIndexed(uint indexCount)
    {
        ...
        GraphicsMetrics.DrawCalls++;
        GraphicsMetrics.TriCount += indexCount / 3;
    }
    ...
    public override void DrawIndexedInstanced(uint indexCount, uint instanceCount)
    {
        ...
        GraphicsMetrics.DrawCalls++;
        GraphicsMetrics.TriCount += indexCount / 3 * instanceCount;
    }
    ...
    private void DebugCallback(GLEnum source, GLEnum type, int id, GLEnum severity, int length, nint message, nint userParam)
    {
        ...
        int logType = debugType switch
        {
            DebugType.DontCare => 0,
            DebugType.DebugTypeError => 2,
            DebugType.DebugTypeDeprecatedBehavior => 2,
            DebugType.DebugTypeUndefinedBehavior => 2,
            DebugType.DebugTypePortability => 1,
            DebugType.DebugTypePerformance => 1,
            DebugType.DebugTypeOther => 0,
            DebugType.DebugTypeMarker => 0,
            DebugType.DebugTypePushGroup => 0,
            DebugType.DebugTypePopGroup => 0,
            _ => throw new ArgumentOutOfRangeException()
        };

        if (logType == 0)
            Prowl.Runtime.Debug.Log($"OpenGL: {msg}");
        else if (logType == 1)
            Prowl.Runtime.Debug.LogWarning($"OpenGL: {msg}");
        else
            Prowl.Runtime.Debug.LogError($"OpenGL: {msg}");
    }
}