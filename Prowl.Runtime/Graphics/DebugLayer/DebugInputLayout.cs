...
internal sealed class DebugInputLayout : InputLayout
{
    ...
    public DebugInputLayout(InputLayoutDescription[] descriptions)
    {
        ...
        foreach (InputLayoutDescription description in descriptions)
        {
            ...
            if (description.Offset != stride)
                Debug.LogError($"Invalid usage at position {position}: An offset of {description.Offset} was found, however an offset of {stride} was expected.");

            uint bitsPerPixel = (uint) description.Format.BitsPerPixel() / 8;

            //if (description.Offset - stride != bitsPerPixel)
            //    Debug.LogWarning($"Potential invalid usage at position {position}: The offset difference was {description.Offset - stride}, however an offset difference of {stride} was expected.");
    ...
    public override void Dispose()
    {
        ...
        Debug.Log("Input layout disposed.");
    }
}